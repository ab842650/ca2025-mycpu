...
class IF2ID extends Module {
  ...
  val instruction = Module(new PipelineRegister(defaultValue = ProgramCounter.EntryAddress))
  ...
  // TODO: Complete the instantiation and connection
  val instruction_address = Module(new PipelineRegister(defaultValue = InstructionsNop.nop))
